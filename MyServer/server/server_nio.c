...
{
	logCrit("storage recv notify read ! sock:%d event:%d",sock,event);
	...
	while (1)
	{
		logCrit("in while of storage_recv_notify_read");
		if ((bytes=read(sock, &task_addr, sizeof(task_addr))) < 0)
		{
			...
			logCrit("read from pipe ! errno == EAGAIN || errno == EWOULDBLOCK");
			break;
		}
		else if (bytes == 0)
		{
		...
		logCrit("the bytes of read from pipe");
		pTask = (struct fast_task_info *)task_addr;
		pClientInfo = (StorageClientInfo *)pTask->arg;

		if (pTask->event.fd < 0)  //quit flag
		{
			logError("pTask->event.fd < 0 ! quit flag");
			return;
		}
		...
		logCrit("pClientInfo->stage = %d",pClientInfo->stage);
		switch (pClientInfo->stage)
		{
			...
			case FDFS_STORAGE_STAGE_NIO_RECV:
				logCrit(" FDFS_STORAGE_STAGE_NIO_RECV ");
				pTask->offset = 0;
				...
				logCrit("quit FDFS_STORAGE_STAGE_NIO_RECV");
				result = 0;
				break;
...
static void client_sock_read(int sock, short event, void *arg)
{
	...
	if (pClientInfo->stage != FDFS_STORAGE_STAGE_NIO_RECV)
	{
		...
		logCrit("return because of pClientInfo->stage != FDFS_STORAGE_STAGE_NIO_RECV");
		return;
	}
	...
	while (1)
	{
		logCrit("in while of client_sock_read");
		...
		*/
		...
		logCrit("recv data %d in client_sock_read", bytes);
		if (bytes < 0)
		{
			if (errno == EAGAIN || errno == EWOULDBLOCK)
			{
			...
			{
				logCrit("errno == EINTR, no data recv, continue wait recv data");
				continue;
			}
			else
			{
			...
			logCrit("errno == EAGAIN || errno == EWOULDBLOCK");
			return;
		}
		else if (bytes == 0)
		{
		...
		{
			logCrit("pClientInfo->total_length == 0 // header");
			if (pTask->offset + bytes < sizeof(TrackerHeader))
			{
				pTask->offset += bytes;
				logCrit("haven't recv done struct of TrackerHeader");
				return;
			}
		...
		{
			logCrit("pTask->offset >= pTask->length //recv current pkg done");
			if (pClientInfo->total_offset + pTask->length >= \
					pClientInfo->total_length)
			{
				/* current req recv done */
				logCrit("current req recv done");
			...
			else
			{
				...
				logCrit("should be continue write to file ! but there are nothing");
				//storage_dio_queue_push(pTask);
				//task_finish_clean_up(pTask);  zi ji jia de  dai ma 
			}
			logCrit("quit client_sock_read!");
			return;
		}
	}
	return;
